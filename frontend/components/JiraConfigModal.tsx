  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl p-8 w-full max-w-md space-y-5">
        <div>
          <h2 className="text-xl font-bold text-white">⚙️ Kết nối Jira</h2>
          <p className="text-gray-400 text-sm mt-1">
            Chỉ cần setup một lần — tự động dùng cho mọi export sau này
          </p>
        </div>

        <div className="space-y-3">
          <div>
            <label className="text-gray-400 text-sm">Jira Site URL</label>
            <input
              className="w-full bg-gray-800 rounded-lg px-3 py-2 text-sm text-white mt-1"
              placeholder="https://yourcompany.atlassian.net"
              value={form.jira_site_url}
              onChange={(e) => setForm({ ...form, jira_site_url: e.target.value })}
            />
          </div>
          <div>
            <label className="text-gray-400 text-sm">API Token</label>
            <input
              type="password"
              className="w-full bg-gray-800 rounded-lg px-3 py-2 text-sm text-white mt-1"
          ...
          <div>
            <label className="text-gray-400 text-sm">Project Key</label>
            <input
              className="w-full bg-gray-800 rounded-lg px-3 py-2 text-sm text-white mt-1"
              placeholder="SCRUM"
        ...
        <button
          onClick={handleSave}
          disabled={saving || !form.jira_site_url || !form.jira_api_token || !form.jira_project_key}
